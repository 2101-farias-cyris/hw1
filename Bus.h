        ...
        for (int i = 0; i < this->persons.size(); i++) {
            for (int j = 0; j < this->persons.size() - i - 1; j++) {
                if (this->persons.at(j).getHeight() < this->persons.at(j + 1).getHeight()) {
                    ...
                    this->persons.at(j) = this->persons.at(static_cast<std::vector<Person, std::allocator<Person>>::size_type>(j) + 1);
                    this->persons.at(static_cast<std::vector<Person, std::allocator<Person>>::size_type>(j) + 1) = temp;
                }
            }
        }
    }
